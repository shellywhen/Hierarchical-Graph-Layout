...
function barChartView(){
  ...
  var maxx=0,minx=999999,delta=0,maxy=0;
  var degree=[],count=[];
  var stat=[];
  var cut=[],height=[],bucket=new Int32Array(99999);
  for(var i in node){
...
var svg=d3.select("#barChart").append("svg").attr("width",$("#barChart").css("width")).attr("height",$("#barChart").css("height")),
    g= svg.append("g").attr("transform", "translate("+margin.left +"," +margin.top+")");
var x =d3.scaleLog().rangeRound([0,width]).domain([minx,maxx]);
var bins=d3.histogram().domain([minx,maxx]).thresholds(x.ticks(30))(stat);
console.log(bins);
...
bar.append("rect")
   .attr("x", 1)
   .attr("width",function(d,i){return (x(bins[i].x1) - x(bins[i].x0));})
   .attr("height", function(d) { return height - y(d.length); });
 bar.append("text")
    .attr("dy", ".75em")
    .attr("y",- 10)
    .attr("x", function(d,i){return (x(bins[i].x1) - x(bins[i].x0))/2;})
